      ...
      <b-row>
        <b-col cols="12">
          <b-card
            ...
            footer-tag="footer">
            <div slot="header">
              <i class="fa fa-align-justify"></i><strong> Add New Category</strong>
...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      this.$store.dispatch('SAVE_SUPPLIERTYPE', {
        data: data,
        config
      });
    },
    ...
    userLink(_id) {
      return `addsupplier-type/${_id.toString()}`;
    },
    rowClicked(item) {
  ...
  computed: {
    ...mapState({
      suppliertypes: state => state.suppliertype
    }),
    items() {
